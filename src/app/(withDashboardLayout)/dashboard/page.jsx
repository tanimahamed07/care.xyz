...
const DashboardOverview = () => {
  ...
  // স্ট্যাটিক কাউন্টার ডাটা (পরবর্তীতে API থেকে আসবে)
  const stats = [
    {
      id: 1,
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="text-center md:text-left space-y-4">
            <h1 className="text-3xl md:text-5xl font-black leading-tight">
              Hello, <span className="text-primary italic">{session?.user?.name || "User"}!</span>
            </h1>
            <p className="text-neutral-content/70 max-w-md text-lg">
              Your loved ones are in safe hands. Check your upcoming schedules and care progress here.
            </p>
            <div className="pt-2">
              <Link href="/services" className="btn btn-primary rounded-xl px-8 shadow-lg shadow-primary/30 border-none">
                Book New Service
              </Link>
            </div>
          </div>
          ...
          <div className="hidden lg:flex gap-4">
            <div className="bg-white/10 backdrop-blur-md p-6 rounded-3xl border border-white/10 text-center w-32">
              ...
              <span className="text-[10px] uppercase font-bold tracking-widest opacity-60">Reviews</span>
            </div>
            <div className="bg-white/10 backdrop-blur-md p-6 rounded-3xl border border-white/10 text-center w-32 mt-8">
              ...
              <span className="text-[10px] uppercase font-bold tracking-widest opacity-60">Avg Rating</span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <div key={stat.id} className="bg-base-100 p-6 rounded-[2rem] border border-base-200 shadow-sm hover:shadow-md transition-all group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-base-100 rounded-[2.5rem] border border-base-200 p-8 shadow-sm">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl font-black text-neutral">Recent <span className="text-primary italic">Bookings</span></h2>
            <Link href="/dashboard/my-bookings" className="text-sm font-bold text-primary hover:underline flex items-center gap-1">
          ...
          <div className="space-y-4">
            {/* Single Booking Item */}
            {[1, 2].map((item) => (
              <div key={item} className="flex flex-col sm:flex-row items-center justify-between p-5 rounded-2xl bg-base-200/50 border border-transparent hover:border-primary/20 transition-all">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center shadow-sm">
                    <FaHandHoldingHeart className="text-primary" />
                  </div>
                  <div>
                    <h4 className="font-bold text-neutral">Early Childhood Care</h4>
                    <p className="text-xs text-neutral/50">Scheduled for: 24 Dec, 2025</p>
                  </div>
                </div>
                <div className="mt-4 sm:mt-0 flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
                  <span className="badge badge-warning badge-sm font-bold p-3">PENDING</span>
        ...
        <div className="bg-accent/5 rounded-[2.5rem] border border-accent/10 p-8">
          <h3 className="text-xl font-black text-neutral mb-4">Quick <span className="text-accent italic">Support</span></h3>
          <p className="text-sm text-neutral/60 mb-6">Need help with your booking or have questions about a caregiver?</p>
          ...
          <div className="mt-8 p-4 bg-white rounded-2xl border border-accent/20">
            <p className="text-[10px] uppercase font-black text-accent tracking-tighter mb-1">Safety Note</p>
            <p className="text-xs text-neutral/70 italic">"Always verify the NID of the caregiver upon arrival."</p>
          </div>
        </div>
